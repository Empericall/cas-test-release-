package repository.impl;

import com.google.inject.Inject;
import com.google.inject.Singleton;
import db.mapper.CarModelsDbMapper;
import lombok.RequiredArgsConstructor;
import model.entity.CarModelEntity;
import repository.CarModelsRepository;
import java.util.List;
import java.util.concurrent.CompletableFuture;
import java.util.concurrent.CompletionStage;

@Singleton
@RequiredArgsConstructor(onConstructor_ = { @Inject})
public class CarModelsRepositoryImpl implements CarModelsRepository {

    private final CarModelsDbMapper mapper;

    @Override
    public CompletionStage<CarModelEntity> findById(Long id) {
        return CompletableFuture.supplyAsync(() -> mapper.findById(id));
    }

    @Override
    public CompletionStage<List<CarModelEntity>> findAll() {
        return CompletableFuture.supplyAsync(() -> mapper.findAll());
    }

    @Override
    public CompletionStage<Void> deleteById(Long id) {
        return CompletableFuture.runAsync(() -> mapper.deleteById(id));
    }

    @Override
    public CompletionStage<Void> insert(CarModelEntity entity) {
        return CompletableFuture.runAsync(() -> mapper.insert(entity));
    }

    @Override
    public CompletionStage<Void> update(CarModelEntity entity) {
        return CompletableFuture.runAsync(() -> mapper.update(entity));
    }
}
